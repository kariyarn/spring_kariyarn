   ...
   <script>
      ...
      document.querySelector("#submitBtn").addEventListener("click", ()=>{
    	 ...
    	 if(caption.length < 3 || files.length == 0){
    		 alert("caption을 3글자 이상 입력하고 업로드할 이미지를 선택해 주세용");
    		 return; //함수를 여기서 종료
    	 }
         //ajax 전송할 데이터를 formData() 객체에 담는다.
         const data = new FormData();
         //caption이라는 파라미터 명으로 input 요소에 입력한 문자열을 담는다.
         ...
         /* 
         	fetch()함수를 호출하면서 method:"post", body: FormatData 객체를 전달하면
         	<forM method="post" enctype="multipart/form-data">폼을 전송한 것과 같다.
         	jsp에서는 getParameter로 읽지 못하지만 Spring에서는 읽어오는 것이 가능하다.
         */
         ...
         .then(data=>{
        	 console.log(data);
        	 if(data.isSuccess){
        		 alert(file.name+" 이미지를 성공적으로 업로드했습니다.");
        		 //UI를 초기상태로 되돌리기
      ...
      dropZone.addEventListener("drop", (e)=>{
         ...
         console.log(files[0]);
         //drop된 파일의 정보를 조사해서 이미지 파일이 아니면 함수를 여기서 종료 시키기
